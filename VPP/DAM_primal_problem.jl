...
generator_connections = Dict()

# Loop through the generator data to extract connections
for (gen_id, gen_info) in gen_data
  println(gen_info)
    # bus_id = gen_info["bus_id"]  # "bus" gives the bus to which the generator is connected
    # generator_connections[gen_id] = bus_id
end

# Display the generator-to-bus connections
println(generator_connections)

x = Dict("a"=>"A", "b"=>"B", "c"=>"C")

for (key, value) in x
    println(key); println(value)
end
function get_incidence_matrix(Dict)
